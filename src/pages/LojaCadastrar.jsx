...
export default function LojaCadastrar() {
  ...
  async function cadastrar() {
    ...
    if (error) {
      setErro(error.message.includes('already') ? 'Este e-mail ja esta cadastrado.' : 'Nao foi possivel criar a conta.')
      setCarregando(false)
      return
    }
  ...
  if (sucesso) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', fontFamily: 'Arial, sans-serif', background: '#fff', padding: 28, textAlign: 'center' }}>
        <div style={{ fontFamily: 'Georgia, serif', fontSize: 26, letterSpacing: 4, color: '#AA1B2F', marginBottom: 20 }}>CLOSET NALU</div>
        <div style={{ fontSize: 16, color: '#333', marginBottom: 10 }}>Conta criada!</div>
        <div style={{ fontSize: 13, color: '#777', maxWidth: 300, marginBottom: 28 }}>Enviamos um e-mail de confirmacao para {email}. Confirme para ativar sua conta.</div>
        <Link to="/entrar" style={{ color: '#AA1B2F', textDecoration: 'none', fontSize: 14, fontWeight: 500 }}>Ir para o login</Link>
      </div>
    )
  }
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', fontFamily: 'Arial, sans-serif', background: '#fff' }}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '30px 28px', maxWidth: 380, margin: '0 auto', width: '100%' }}>
        ...
        <div style={{ fontSize: 12, letterSpacing: 2, color: '#aaa', textTransform: 'uppercase', marginBottom: 32 }}>Criar sua conta</div>

        <div style={{ width: '100%', marginBottom: 14 }}>
          <label style={{ fontSize: 12, color: '#888', display: 'block', marginBottom: 6 }}>Nome</label>
          <div style={{ display: 'flex', alignItems: 'center', border: '0.5px solid #ddd', borderRadius: 8, padding: '0 12px', height: 44 }}>
            <input value={nome} onChange={(e) => setNome(e.target.value)} placeholder="Seu nome" style={{ border: 'none', outline: 'none', flex: 1, fontSize: 14 }} />
          </div>
        </div>
        ...
        <div style={{ width: '100%', marginBottom: 24 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', border: '0.5px solid #ddd', borderRadius: 8, padding: '0 12px', height: 44 }}>
            <input type={verSenha ? 'text' : 'password'} value={senha} onChange={(e) => setSenha(e.target.value)} placeholder="minimo 6 caracteres" style={{ border: 'none', outline: 'none', flex: 1, fontSize: 14 }} />
        ...
        <button onClick={cadastrar} disabled={carregando} style={{ width: '100%', height: 46, background: '#AA1B2F', color: '#fff', border: 'none', borderRadius: 8, fontSize: 15, letterSpacing: 1, cursor: 'pointer', opacity: carregando ? 0.6 : 1 }}>
        ...
        <button onClick={cadastrarGoogle} style={{ width: '100%', height: 46, background: '#fff', color: '#333', border: '1px solid #ddd', borderRadius: 8, fontSize: 14, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
        ...
        <div style={{ fontSize: 13, color: '#666', marginTop: 24, textAlign: 'center' }}>
          Ja tem conta? <Link to="/entrar" style={{ color: '#AA1B2F', textDecoration: 'none', fontWeight: 500 }}>Entrar</Link>
        </div>
      </div>
    </div>
  )
